function numberPlateExtract
%NUMBERPLATEEXTRACTION extracts the characters from the input number plate image.

% im=imread('DNP.jpg','jpg');% Reading the number plate image.
im =imread('dutch2test.bmp ','bmp'); 
imshow(im);
im2=imresize(im,[400 NaN]); % Resizing the image keeping aspect ratio same.
im3=rgb2gray(im2); % Converting the RGB (color) image to gray (intensity).
im3=medfilt2(im3,[3 3]); % Median filtering to remove noise.
se=strel('disk',1); % Structural element (disk of radius 1) for morphological processing.
gi=imdilate(im3,se); % Dilating the gray image with the structural element.
ge=imerode(im3,se); % Eroding the gray image with structural element.
...
Binary=logical(gdiff); % Conversion of the class from double to binary. 
% Eliminating the possible horizontal lines from the output image of regiongrow
% that could be edges of license plate.
er=imerode(Binary,strel('line',50,0));
out1=imsubtract(Binary,er);
